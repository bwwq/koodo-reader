...
class Header extends React.Component<HeaderProps, HeaderState> {
  ...
  startScheduledSync = () => {
    ...
    this.scheduledSyncTimer = setInterval(async () => {
      ...
      if (!this.state.isSync && !this.isSyncing) {
        const serviceUser = await this.props.handleFetchServiceUser();
        await this.handleCloudSync(serviceUser);
      }
    }, intervalMs);
  };
  ...
  handleCloudSync = async (serviceUser: any): Promise<false | undefined> => {
    if (this.isSyncing) {
    ...
    try {
      ...
      let res = await this.beforeSync(serviceUser);
      if (!res) {
  ...
  handleSuccess = async () => {
    ...
    await ConfigUtil.updateSyncData();
